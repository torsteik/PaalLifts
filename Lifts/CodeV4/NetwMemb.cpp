...
int NetwMemb::send(char msg_content[BUFF_SIZE]){
	//printf("Sending msg with ID: %i and first data: %i\n", msg_content[0], msg_content[1]);

	socklen_t send_addr_size = sizeof(this->send_addr);
	if (sendto(this->sock_fd, msg_content, BUFF_SIZE, 0, (struct sockaddr*) &(this->send_addr), send_addr_size) < 0){
